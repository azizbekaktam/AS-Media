...
export function CartoonDetailPage() {
  ...
  useEffect(() => {
    async function fetchCartoonData() {
      if (!id) return;
      ...
      try {
        ...
        const movieWithType = { ...movieData, type: 'multfilm' };
        ...
        if (user && movieData) {
          await addToHistoryWithDocId(user, movieWithType, 'multfilm');
        }
      } catch (error) {
        console.error('Error fetching cartoon:', error);
        setError('Multfilmlarni yuklashda xatolik yuz berdi.');
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-neutral-950 text-white">
        <LoadingSpinner text="Multfilm yuklanmoqda..." />
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-neutral-950 text-white">
        <div className="text-center">
          <div className="text-red-500 text-xl mb-4">{error || 'Multfilm topilmadi'}</div>
          <button 
        ...
        <GlassCard className="flex flex-col md:flex-row gap-10 mt-10 p-6">
          <div className="w-full md:w-[320px] lg:w-[360px] overflow-hidden rounded-2xl shadow-2xl">
            <img
              ...
              alt={movie.title || 'Cartoon Poster'}
